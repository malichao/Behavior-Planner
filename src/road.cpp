...
Vehicle Road::get_ego() { return this->vehicles.find(this->ego_key)->second; }
...
void Road::cull() {
  ...
  while (it != this->vehicles.end()) {
    int v_id = it->first;
    Vehicle v = it->second;
    if ((v.s > (center_s + this->update_width / 2)) ||
      ...
      while (!placed) {
        ...
        if (claimed.find({lane_num, s}) != claimed.end()) {
          ...
          cout << "adding vehicle " << this->vehicles_added << " at lane "
               << lane_num << " with s=" << s << endl;
        }
      }
    }
    it++;
